...
public abstract class CommonClassifierTest {
    ...
    public void setUp() throws SQLException {
        ...
        dataset.setTagListByOffset(START_POSITION_OF_POPULAR_TAGS, LIMIT_NUMBER_OF_POPULAR_TAGS);
    }
    ...
    private Instances getNormalizedInstances() {
        ...
        try {
            normalizedPostIds = dataset.getNormalizedIds();
        ...
        try {
            allnGrammsFromDB = dataset.getAllNGramsFromDB(normalizedPostIds);
        ...
        try {
            instances = dataset.getDataset(normalizedPostIds, allnGrammsFromDB);
     ...
     */
    private void getTrainAndTestSet(final List<Long> normalizedIds, Set<String> allnGrammsFromDB) throws SQLException {
        dataset.splitToTrainAndTest(normalizedIds, CV_RATIO_TEST);

        logger.info("Training set: ");
        trainingSet = dataset.getDataset(dataset.getNormalizedIdsTrain(), allnGrammsFromDB);
        ...
        testingSet = dataset.getDataset(dataset.getNormalizedIdsTest(), allnGrammsFromDB);
