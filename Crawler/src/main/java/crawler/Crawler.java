...
public class Crawler {
    ...
    private static final Integer MAX_TAG_USES_POW = 6; // max uses count 1e6

    // set of all users
    private static Set<String> allUsers;

    // queue of users who should be considered
    private static LinkedList<String> usersQueue;

    // dictionary where key=tags' name; value=count uses of this tags
    private static Map<String, Integer> allTags;
    ...
    private static void getUserFriends(String user) throws UnirestException, InterruptedException {
    ...
    // get all user's tags
    private static List<Tag> getUserTags(String user) throws UnirestException, InterruptedException {
    ...
    public void crawl(String startUser) {
        ...
        while(!usersQueue.isEmpty()) {
            ...
            try {
                ...
                System.out.println("I don't no why do you see this exception.");
                e.printStackTrace();
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            ...
            try {
                ...
                File file = new File("Crawler\\src\\main\\resources\\users\\" + user + ".txt");
                if (!file.exists()) {
                    file.createNewFile();
                }
        ...
        try {
            ...
            File file = new File("Crawler\\src\\main\\resources\\tags.txt");
            if (!file.exists()) {
                file.createNewFile();
            }
